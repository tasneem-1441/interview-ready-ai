...
function PreparePage() {
    ...
    async (answers: AnswerRecord[]) => {
      ...
      if (job && weakest) {
        try {
          ...
          // Fall back to built-in diagnostic tasks generated by buildReport
        }
      }

      update({ answers, report: built, step: 3, maxReached: Math.max(maxReached, 3) });
      setScoring(false);
      setPending(null);
  ...
  const scoreWithoutAi = useCallback(() => {
    ...
    update({ answers: pending, report: built, step: 3, maxReached: Math.max(maxReached, 3) });
    setScoreError(null);
    setPending(null);
    if (job) {
      recordAttempt({
  ...
  // Guard: a step is only shown when the data it depends on actually exists.
  const safeStep =
